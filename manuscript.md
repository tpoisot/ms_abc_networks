...
of approaches [@Nee06; @AldoPopo05]. At their core, most approaches are
...
both an ecological [@GravMass11; @CazeArau15] and evolutionary [@EkloHelm12;
@EkloStou16; @StouSale12] standpoint since it is widely accepted that
...
drive properties like community persistence or ecosystem function [@ThomBros12].
...
interactions [@GuisThui05; @ThuiMunk13], such models are still unable to predict
the structure of complex interacting communities [@Jabl08].  Nevertheless, there
are two key observations upon which solutions to overcome this limitation can be
...
macro-evolutionary processes [@StouSale12]. Second, the structure of ecological
networks is dynamic over evolutionary timescales [@McKaDros05]. Both these
points are strongly suggestive of perpetual and ongoing action of
macro-evolutionary processes. It stands to reason then that models of
macro-evolution with explicit consideration of species interactions will
therefore provide an appropriate theoretical framework to understand how
networks evolve. Notably, such a framework enables the estimation of how much of
extant network structure originated through macro-evolution, as opposed to
reflecting extant opportunities and constraints [@Pera16].
...
has been used to study species diversification [@Bent15]. Unfortunately, the
...
interactions, and there is a wealth of well-studied data available [@Will11].
...
ecology [@DelmBess17; @AntoDatt18], we have focused our analysis on measures
that have a number of desirable properties. First; they return a single value,
and therefore function as a "summary" of an aspect of network structure. Second;
...
We measured nestedness, using the $\eta$ measure of @BastFort09, which returns a
...
We measured modularity using the BRIM approach [@Barb07] (preliminary analyses
...
Finally, we enumerated six four-species bipartite motifs [@BakeKaar14].
Bipartite motifs are possible conformations of four species spread across two
...
second numerical experiment resembles empirical communities [@Beau10]. For each
...
not meaningful [@SunnBuse13]. To circumvent this issue, we adopted a two-steps
...
The model (and all the data analysis code) was written in `Julia` [@BezaEdel17]
...
All networks show a strong bias towards moderately high values of $c$; this
indicates that the effective probability of a species retaining its ancestor's
interactions decreases with its ancestor's degree. That is, the generalism of
species over time has an emergent upper bound, a fact that results in the very
spectrum of high-degree and low-degree species that is ubiquitous empirically
[@Will11]. We are able top re

Because some structural properties of networks emerge in the absence of rules on
evolutionary traits, @ValvPine18 have suggested that they represent evolutionary
spandrels. @MaynServ18 show that these spandrels tie into ecological rules on
...
ecological communities eventually tend. @CoelRang18 recently show that a
*neutral* model, in which all species have the same properties (as in ours) is
able to reproduce the structure of spatialized ecological networks.

Taken together, our results and the recent literature suggest that there is only
a reduced ensemble of evolutionary histories that are able to generate realistic
ecological networks -- as opposed to previous suggestions [*e.g.* @WillMart00]
none of these models require species-level information to make accurate
predictions. The ability to observe good predictions without knowledge about
species is not surprising in the light of recent empirical results. @PoniMGo17
and @PoisStou18 report (using some networks part of the present analysis) that
the cophylogenetic signal is relatively weak on networks, but much stronger at
the scale of single interactions. This suggests that when interested in a
bird's-eye view of the network, as described by properties such as connectance,
modularity, etc., the identity and traits of species matters very little
[@HeleGarc14].
...
interactions [@Thom94], this can cascade up to the macro-evolutionary scale in
